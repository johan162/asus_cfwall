...
if [ ! -e whitelist.ipset  ]; then
    ...
    ipset   -N whitelist nethash --hashsize 82944 --probes 4 --resize 50

    # Allow reserved broadcast and VPN networks to passes over eth1
    ipset   -A whitelist 0.0.0.0/8
    ipset   -A whitelist 10.0.0.0/8

    for c in  $(cat white.conf) ; do
        logger  "CYDROP: Adding $c to whitelist ..."
        for ip in $(cat $c-aggregated.zone ); do ipset   -A whitelist $ip; done
    done
...
if [ $? -eq 1 ]; then
    ...
    iptables   -I FORWARD 3 -p ALL -i eth0 -m set ! --set whitelist src -j CYDROP

    iptables   -I INPUT 1 -m state --state RELATED,ESTABLISHED -j ACCEPT
    iptables   -A INPUT -p ALL -i eth0 -m set ! --set whitelist src -j CYDROP

    logger  "CYDROP: Whitelist sucessfully installed" >&2
else
    logger  "CYDROP ERROR: Whitelist chin already installed. No changes made" >&2
    exit 1
fi
